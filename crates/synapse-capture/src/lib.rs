...
pub fn captured_frame_region_to_software_bitmap(
    ...
    windows_impl::captured_frame_region_to_software_bitmap(frame, region)
}
...
pub fn screen_region_to_software_bitmap(
    ...
    windows_impl::screen_region_to_software_bitmap(region)
}
...
#[cfg(windows)]
mod windows_impl {
    use std::{cell::RefCell, ffi::c_void, slice, thread, time::Duration};
    ...
    use windows::{
        ...
        Win32::{
            ...
            Graphics::{
                ...
                Gdi::{
                    BI_RGB, BITMAPINFO, BITMAPINFOHEADER, BitBlt, ClientToScreen,
                    CreateCompatibleDC, CreateDIBSection, DIB_RGB_COLORS, DeleteDC, DeleteObject,
                    GetDC, HBITMAP, HDC, HGDIOBJ, ReleaseDC, SRCCOPY, ScreenToClient, SelectObject,
                },
            },
            System::Threading::{
    ...
    use super::{
        ...
        CapturedFrame, CapturedSoftwareBitmap, DpiAwarenessStatus, DxgiFormat, SendablePtr,
        push_frame,
    };
    ...
    fn copy_mapped_rows(
        ...
        let row_pitch = usize::try_from(mapped.RowPitch).map_err(|err| {
            CaptureError::GraphicsApiUnsupported {
                detail: err.to_string(),
            }
        })?;
