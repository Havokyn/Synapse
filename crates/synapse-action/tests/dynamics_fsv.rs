...
fn burst_schedule_has_zero_iki_and_empty_input_stays_empty() {
    ...
    println!(
        "source_of_truth=dynamics_schedule edge=empty before={empty_before:?} after={empty:?} final_value={}",
        empty.len()
    );
    ...
    println!(
        "source_of_truth=dynamics_schedule edge=burst before={text:?} after={schedule:?} final_value={ikis:?}"
    );
...
fn linear_schedule_spaces_after_first_character_and_records_shift() {
    ...
    let modifiers: Vec<_> = schedule
        ...
        .collect();
    println!(
        "source_of_truth=dynamics_schedule edge=linear_shift before={text:?} after={schedule:?} final_value=ikis:{ikis:?},modifiers:{modifiers:?}"
    );
...
fn natural_schedule_applies_compile_time_bigram_bias_and_seed_determinism() {
    ...
    let different_seed_ikis: Vec<_> = different_seed
        ...
        .collect();
    println!(
        "source_of_truth=dynamics_schedule edge=natural_seeded before={text:?} after={first:?} final_value=ikis:{ikis:?},different_seed:{different_seed_ikis:?}"
    );
...
fn natural_schedule_sanitizes_non_finite_params_without_panicking() {
    ...
    let ikis: Vec<_> = schedule.iter().map(|event| event.iki_ms_before).collect();
    println!(
        "source_of_truth=dynamics_schedule edge=non_finite before={text:?} after={schedule:?} final_value={ikis:?}"
    );
