...
fn hardware_backend_resolves_to_fail_closed_stub() {
    ...
    println!(
        "source_of_truth=hardware_resolution edge=act_press before_backend={before:?} after_backend={} final_value={after:?}",
        after.as_str()
    );
}
...
fn hardware_unavailable_backend_rejects_every_action_variant() {
    ...
    for (edge, action) in action_cases() {
        ...
        println!(
            "source_of_truth=hardware_m2 edge={edge} before={before:?} after={after:?} after_code={} detail={}",
            error.code(),
            error.detail()
        );
    }
