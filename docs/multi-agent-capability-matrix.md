...
| browser_evaluate | browser perception/action | active session CDP target or CDP target owned by this MCP session | raw CDP Runtime.evaluate in the page main world (await_promise/return_by_value) with separate URL/title/readyState readback; JS exceptions are surfaced loudly with class/message/location; the popup-safe normal Chrome extension bridge fails closed because it never attaches the debugger | no foreground lease, no tab activation, no implicit foreground tab fallback, refuses unowned/non-active targets | background-pass | #1065 #1067 #1063 | evaluated JSON value plus Runtime.RemoteObject type/subtype/description readback and CF_ACTION_LOG |
